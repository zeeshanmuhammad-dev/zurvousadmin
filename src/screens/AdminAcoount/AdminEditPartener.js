...
import {editPar} from '../../actions/actions'

const AddEditPartener = (props) => {
  ...
  function addUser(){
    ...
    PostReq(JSON.stringify(dataJson),ApiRoute.addPartener+"/"+props.route.params.id).then(res => {
      //console.log("sss",res)
        alert(JSON.stringify(res.message))
        props.editPar({id:props.route.params.id,full_name:fullName,email})
        props.navigation.navigate('AdminAddPartener')
      setLoading(false)
    }).catch(()=>{
      setLoading(false)
    })
      ...
      <View style={Styles.innerContainer}>
        <View style={Styles.inputFields}>
          ...
          <TouchableNativeFeedback
            ...
            onPress={() => addUser()}>
            <Text style={Styles.loginText}>UPDATE PARTENER</Text>
          </TouchableNativeFeedback>
        </View>
      </View>
    </View>
...
const mapDipatchToProps=(dispatch)=>{
  return{
    editPar:(inf)=> dispatch(editPar(inf))
  };
}

export default connect(mapStateToProps,mapDipatchToProps)(AddEditPartener);
