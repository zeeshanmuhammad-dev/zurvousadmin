...
const EditFaq = (props) => {
  const [faqType, setFaqType] = useState(props.route.params.type);
  const [loading,setLoading] =useState(false);
  const [question, setQuestion] = useState(props.route.params.question);
  const [answer, setAnswer] = useState(props.route.params.answer);
  ...
  function addFeqQuestion(){
    ...
    PostReq(JSON.stringify(dataJson),ApiRoute.editFaq).then(res => {
...
{video!=='0'||video!==''?
<VideoPlayer
                                                            source={{ uri: Platform.OS === 'ios' ? video.uri : video.uri }}
                                                            ref={videoRef}
          ...
          <TouchableNativeFeedback
            ...
            <Text style={Styles.loginText}>UPDATE QUESTIONS</Text>
          </TouchableNativeFeedback>
        </View>
      </ScrollView>
    </View>
