...
const AdminManageGymNewEntry = ({ navigation }) => {
  const textRef = useRef();
  const [searchValue, setSearchValue] = useState('');
  const [existingGym, setexistingGym] = useState('');
  ...
  function searchUpdated(term) {
    setSearchTerm(term)
    setFilter(existingGym.filter(createFilter(term, KEYS_TO_FILTERS)))
  }
  ...
  function newGymEntry() {
    getRequest(ApiRoute.newGyms).then(res => {
    
      setexistingGym(res.data)
      setisVisible(false)
    })
  }
  ...
  return (
    ...
    <View style={Styles.container}>
      ...
      <FlatList
        data={filter===''?existingGym:filter}
...
export default AdminManageGymNewEntry;
