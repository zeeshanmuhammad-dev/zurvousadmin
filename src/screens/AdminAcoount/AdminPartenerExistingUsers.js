...
import {setPar} from '../../actions/actions'
const AdminPartenerExistingUsers = ({navigation,setPar,pars}) => {
  ...
  function getPartners(){
    setLoading(true)
    getRequest(ApiRoute.getPartners).then(res => {
  ...
  const hideMenu = (postion,index)  => {
    setLoading(true)
    getRequest(ApiRoute.deletePartner+postion,true).then(res=>{
      alert(res.message)
  ...
  const viewDetail=(id,index)=>{
    navigation.navigate('PartnerDetailScreen',{id});
    menuRef.current[index].hide();
  }
  ...
  const editItem=(item,index)=>{
    navigation.navigate('EditPartnerScreen',{id:item.id,fullName:item.full_name,email:item.email,
  ...
  return (
    ...
    {pars.map((item,index)=><View style={Styles.contentView}>
...
const mapDipatchToProps=(dispatch)=>{
  return{
    setPar:(inf)=> dispatch(setPar(inf))
  };
}

export default connect(mapStateToProps,mapDipatchToProps)(AdminPartenerExistingUsers);

//export default AdminPartenerExistingUsers;
